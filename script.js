...
const csvFile = "C:/Users/user/Downloads/projects/dashboard-d3/dataset/1900_2021_DISASTERS.xlsx";

async function getData() {
  const response = await d3.csv(csvFile);
  return response;
}
...
async function main() {
  ...
  const yScale = d3.scaleLinear().domain([0, d3.max(data, (d) => +d["Count of Total Affected"])]).range([height, 0]);
  ...
  const leftRect = svg.append("g")
    .selectAll("rect")
    .data(data)
    .enter()
    .append("rect")
    .attr("x", (d) => xScale(d.Country))
    .attr("y", (d) => yScale(+d["Count of Total Affected"]))
    .attr("width", xScale.bandwidth())
    .attr("height", (d) => height - yScale(+d["Count of Total Affected"]))
    .attr("fill", "steelblue");

  const rightRect = svg.append("g")
    .selectAll("rect")
    .data(data)
    .enter()
    .append("rect")
    .attr("x", (d) => xScale(d.Country) + xScale.bandwidth())
    .attr("y", 0)
    .attr("width", xScale.bandwidth())
    .attr("height", (d) => yScale(+d["Count of Total Affected"]))
    .attr("fill", "coral");
